...
def auth():
    ...
    query_results = select_from_users(
        "user_type,cityID",
        f'username = "{username}" AND password = "{hashlib.sha256(str.encode(str(password)+"Alittlebitofsaltandpepper.")).hexdigest()}"',
    )
...
def city():
    if token_valid() and "type" in session and session["type"] == "city":
        return render_template(
            "city/home.html",
            customer_name=select_from_cities("city", f'cityID={session["cityID"]}')[0][
                0
            ].title(),
        )
    else:
